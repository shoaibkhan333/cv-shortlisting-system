...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            ...
            $stmt = $conn->prepare("SELECT id, password FROM users WHERE email = ? AND role = 'manager'");
            $stmt->execute([$email]);
            ...
            if ($stmt->rowCount() === 1) {
                ...
                if (password_verify($password, $user['password'])) {
                    ...
                    redirect('../manager/manager_dash.php');
                } else {
                    $errors[] = "Invalid password";
                }
            } else {
                $errors[] = "Manager account not found";
            }
...
<head>
    ...
    <title>Manager Login - CV Shortlisting System</title>
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <form method="POST" action="" id="loginForm" onsubmit="return validateForm()">
                            ...
                            <div class="d-grid">
                                <button type="submit" class="btn btn-primary">Login as Manager</button>
                            </div>
                        </form>
                        <div class="text-center mt-3">
                            <p>Don't have a manager account? <a href="manager_signup.php">Register here</a></p>
                            <p>Are you a candidate? <a href="login.php">Login as Candidate</a></p>
                        </div>
                    </div>
                </div>
            </div>
